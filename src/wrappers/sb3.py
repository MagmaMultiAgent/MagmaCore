...
class SB3Wrapper(gym.Wrapper):
    def __init__(
        ...
        unit_controller: Controller = None,
        factory_controller: Controller = None,
    ) -> None:
        """
        ...
        assert unit_controller is not None
        assert factory_controller is not None
        ...
        self.unit_action_space = self.unit_controller.action_space
        self.factory_action_space = self.factory_controller.action_space

        self.action_space = spaces.MultiDiscrete([self.unit_action_space.n, self.factory_action_space.n])
    ...
    # here, for each agent in the game we translate their action into a Lux S2 action
        lux_action = dict()
        logging.debug(f'Action lmnao: {action}')
        for agent in self.env.agents:
            if agent in action:
                if agent == 'factory_0':
                    lux_action[agent] = self.factory_controller.action_to_lux_action(
                        agent=agent, obs=self.prev_obs, action=action[agent]
                    )
                else:
                    lux_action[agent] = self.unit_controller.action_to_lux_action(
                        agent=agent, obs=self.prev_obs, action=action[agent]
                    )
            else:
